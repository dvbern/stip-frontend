...
export const <%= camelize(projectName) %>sFeature = createFeature({
  ...
  reducer: createReducer(
    initialState,
    on(<%= classify(projectName) %>Actions.init, (state): State => ({
      ...state,
      loading: true,
      error: undefined
    })),

    on(<%= classify(projectName) %>ApiActions.<%= camelize(name) %>sLoadedSuccess, (state, { <%= camelize(name) %>s }): State => ({
      ...state,
      <%= camelize(name) %>s,
      loading: false,
      error: undefined
    ...
    on(
      <%= classify(projectName) %>ApiActions.<%= camelize(name) %>sLoadedFailure,
